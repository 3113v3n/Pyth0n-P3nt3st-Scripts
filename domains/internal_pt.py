...
class InternalAssessment(NetworkHandler, Bcolors):
    ...
    def initialize_variables(self, mode, output_file):
        ...
        self.mode = mode
        if mode == "scan":
            self.output_file = self.generate_unique_name(output_file, "csv")
        else:
    ...
    def enumerate_hosts(self):
        ...
        """
        self.get_live_ips(mode=self.mode, output=self.output_file)
